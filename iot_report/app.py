...
def fetch_data(table_name, from_date, to_date, time_difference, columns_to_fetch):
    ...
    try:
        ...
        for row_dict in data:
            if 'time' in row_dict and isinstance(row_dict.get('time'), datetime):
...
def get_data_api():
    ...
    other_cols = [col for col in selected_fields if col in all_fetched_columns and col not in ['date', 'time', 'idx']]
