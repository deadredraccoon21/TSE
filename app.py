...
def run_db_task(func, *args, **kwargs):
    ...
    return tpool.spawn(func, *args, **kwargs).get(timeout=30)
...
def _fetch_data_worker(table_name, from_date, to_date, time_difference_minutes, columns_to_fetch):
    ...
    try:
        ...
        valid_cols = [col for col in columns_to_fetch if col in actual_table_columns and col not in ('date', 'time')]
        prefix = [col for col in ('date', 'time') if col in actual_table_columns]
        valid_cols = prefix + valid_cols
...
def log_alarm_to_db(alarm):
    query = "INSERT INTO AlarmHistory (alarm_id, time, message, code, severity, status, acknowledged, resolved_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
    params = (str(alarm.get('id', '')), str(alarm.get('time', '')), str(alarm.get('message', '')), str(alarm.get('code', '')), str(alarm.get('severity', '')), str(alarm.get('status', '')), 1 if alarm.get('acknowledged') else 0, str(alarm.get('resolved_time', '')))
    execute_query(query, params)

def update_alarm_in_db(alarm):
    query = "UPDATE AlarmHistory SET status=?, acknowledged=?, resolved_time=? WHERE alarm_id=?"
    params = (str(alarm.get('status', '')), 1 if alarm.get('acknowledged') else 0, str(alarm.get('resolved_time', '')), str(alarm.get('id', '')))
    execute_query(query, params)

def limit_persistent_alarms(alarms):
    # Strictly limit to 500 most recent alarms to prevent file bloat
    return alarms[-500:] if len(alarms) > 500 else alarms
...
def update_opc_connection_alarm(is_disconnected):
    ...
    with YAML_ACCESS_LOCK:
        ...
        if is_disconnected:
            ...
            has_active = any(str(a.get('code')) == code and a.get('status') != 'Resolved' for a in persistent_alarms)
            if not has_active:
                new_alarm = {
                    'id': str(uuid.uuid4()), 
            ...
            for alarm in reversed(persistent_alarms):
                if str(alarm.get('code')) == code and alarm.get('status') != 'Resolved':
                    ...
                    update_alarm_in_db(alarm)
                    something_changed = True
                    break
                    
        if something_changed:
            persistent_alarms = limit_persistent_alarms(persistent_alarms)
...
class OpcuaSubscriptionHandler:
    ...
    def datachange_notification(self, node, val, data):
        ...
        node_name = self.node_map.get(node_id_str)
        if not node_name:
            node_name = self.node_map.get(str(node.nodeid))
            
        if node_name:
            if isinstance(val, bool): parsed_val = val
            elif isinstance(val, (int, float)): parsed_val = round(val, 1)
            else: parsed_val = val
            
            with self.lock:
                self.pending_updates[node_name] = parsed_val
...
def read_values_periodically():
    global RUNTIME_STORE, OPC_CONNECTED
        ...
        try:
            ...
            with RUNTIME_STORE["lock"]:
                if RUNTIME_STORE["values"]:
                    zero_vals = {k: 0 for k in RUNTIME_STORE["values"]}
                    socketio.emit('update', zero_vals)
        ...
        try:
            ...
            for name, node_id in node_ids_map.items():
                try:
                    ...
                    # Support multiple formats of nodeid string representation
                    node_map[node.nodeid.to_string()] = name
                    node_map[str(node.nodeid)] = name
                except Exception: pass

            if active_nodes:
                handler = OpcuaSubscriptionHandler(node_map)
            ...
            while True:
                if active_nodes:
                    updates = handler.get_and_clear_updates()
                    with RUNTIME_STORE["lock"]:
                        if updates:
                            RUNTIME_STORE["values"].update(updates)
                        full_snapshot = RUNTIME_STORE["values"].copy()
                        
                    # Emit immediately if there are changes, OR periodically (every 2.5s) to keep frontend graphs moving
                    if updates or (loop_count % 5 == 0):
                        socketio.emit('update', full_snapshot)
                        
                    # Lightweight heartbeat to detect disconnects quickly
                    if successful_nodes:
                        successful_nodes[0].get_value()
            ...
            while not connected:
                ...
                try:
                    ...
                    update_opc_connection_alarm(False)
                    socketio.emit('opc_status', {'connected': True})
                except:
                ...
                with YAML_ACCESS_LOCK:
                    ...
                    for rule in alarm_rules:
                        ...
                        if current_trip and not prev_trip:
                            has_active_alarm = any(str(alarm.get('code')) == code and alarm.get('status') != 'Resolved' for alarm in persistent_alarms)
        ...
        try:
            ...
            for alarm in alarms:
                if alarm.get("id") == alarm_id:
                    ...
                    if alarm.get("status") != "Resolved":
                        alarm["status"] = "Acknowledged"
                    update_alarm_in_db(alarm)
                    updated = True
                    break
            if updated:
                alarms = limit_persistent_alarms(alarms)
                data["alarms"] = alarms
...
def get_data_api():
    ...
    rh_data = (fetch_data(dept_config.get('rh_table'), from_date, to_date, time_diff, selected_fields) or []) if dept_config.get('rh_table') else []
    temp_data = (fetch_data(dept_config.get('temp_table'), from_date, to_date, time_diff, selected_fields) or []) if dept_config.get('temp_table') else []
...
_context_cache = {"payload": None, "ts": 0.0}
_CONTEXT_TTL = 5.0

@app.context_processor
def inject_context():
    now = time.monotonic()
    if _context_cache["payload"] is None or (now - _context_cache["ts"]) > _CONTEXT_TTL:
        data = load_data()
        ...
        tse_depts = _get_tse_departments()
    ...
    return _context_cache["payload"]

@app.route('/<submodule>')
def render_submodule(submodule):
    if submodule in ['login', 'report', 'dashboard', 'user_management', 'admin', 'users', 'admins', 'send_report_page', 'drivemapp']: return "Not found", 404
    config_data = load_data()
...
def handle_connect():
    ...
    with RUNTIME_STORE["lock"]:
        ...
        if not OPC_CONNECTED and vals:
            vals = {k: 0 for k in vals}
        if vals: emit('update', vals)
    emit('opc_status', {'connected': OPC_CONNECTED})
    
    with YAML_ACCESS_LOCK:
        alarms = load_alarms_from_json_list()
    emit('alarm_update', alarms)
...
def input_page():
    ...
    if request.method == 'POST':
        ...
        if 'client_name' in request.form:
            ...
            data['ACCESS_TOKEN'] = request.form.get('whatsapp_access_token')
            data['PHONE_NUMBER_ID'] = request.form.get('whatsapp_phone_number_id')
            save_yaml(data)
    ...
    return render_template('iot/input.html',
        ...
        predefined_departments=load_predefined_departments(),
        smtp_server=data.get('SMTP_SERVER', 'mail.tsengg.com'),
        smtp_port=data.get('SMTP_PORT', 587),
        sender_email=data.get('SENDER_EMAIL', 'reports@tsengg.com'),
...
@app.route('/upload_dept', methods=['POST'])
def upload_dept():
    if 'userloggedin' not in session: return jsonify(success=False, message="Unauthorized")
    if 'dept_file' not in request.files: return jsonify(success=False, message="No file part")
    file = request.files['dept_file']
    if file.filename == '': return jsonify(success=False, message="No selected file")
    if file and file.filename.endswith('.html'):
        from werkzeug.utils import secure_filename
        import os
        filename = secure_filename(file.filename)
        save_path = os.path.join(app.root_path, 'templates', 'iot', 'departments', filename)
        file.save(save_path)
        return jsonify(success=True, message=f"Successfully uploaded {filename}!")
    return jsonify(success=False, message="Invalid file format. Please upload an HTML file.")
...
def clear_alarms():
    ...
    try:
        with YAML_ACCESS_LOCK:
            ...
            global LAST_TRIP_STATES
            LAST_TRIP_STATES = {}
        # Clear from SQL as well
        execute_query("DELETE FROM AlarmHistory")
        return jsonify({'success': True})
    except Exception as e: return jsonify({'success': False, 'error': str(e)}), 500

@app.route('/alarm_history', methods=['GET'])
def alarm_history_page():
    if 'userloggedin' not in session: return redirect('/login')
    return render_template('iot/alarm_history.html', user=session['userloggedin'], user_role=session.get('user_role', 'Operator'))
...
def get_alarms_history():
    try:
        ...
        query = """
            SELECT alarm_id, time, message, code, severity, status, acknowledged, resolved_time
            FROM AlarmHistory
            ORDER BY id DESC
            OFFSET ? ROWS FETCH NEXT ? ROWS ONLY
        """
        records = execute_query(query, (offset, limit), fetch='all')
        
        # Also get total count for pagination
        count_record = execute_query("SELECT COUNT(*) FROM AlarmHistory", fetch='one')
        ...
        if records:
            for r in records:
                history.append({
                    ...
                    'resolved_time': r.get('resolved_time')
                })
            
        return jsonify({'success': True, 'alarms': history, 'total': total_count, 'page': page, 'limit': limit})
    except Exception as e:
...
def send_report_page():
    ...
    # Fetch the severity levels from input.yaml (defaults to 0-5 if missing)
    severity_levels = app_config.get("alarm_severity_levels", [0,1, 2, 3, 4, 5])
    
    return render_template('iot/send_report.html', shift_data=shift_report.load_shift_data(BASE_DIR), telegram_group_id=tg_group_id, severity_levels=severity_levels)
...
def api_update_global_config():
    ...
    # Save the new alarm filter settings alongside the existing ones
# Save the new multiple-severity filter settings
    data['global_config'] = {
        'send_time': request.json.get('send_time', '22:00'), 
        'fields': request.json.get('fields', []),
        'include_alarms': request.json.get('include_alarms', True),
        'show_severity_column': request.json.get('show_severity_column', True),
        'selected_severities': request.json.get('selected_severities', [])
    }
            ...
            if configured_time and fields:
                ...
                if current_time >= configured_time:
                    ...
                    if last_sent_key != track_key:
                        ...
                        # Detect if the app was down when the report was due (delayed > 3 minutes)
                        try:
                            sched_dt = datetime.strptime(f"{today_date} {configured_time}", "%d-%m-%Y %H:%M")
                            is_delayed = (now - sched_dt).total_seconds() > 180
                        except Exception:
                            is_delayed = False
...
# =========================================================================
# === DRIVE MAPP — VFD PARAMETER MANAGEMENT                            ===
# =========================================================================
DRIVE_MAPP_CONFIG_PATH = os.path.join(BASE_DIR, "drive_mapp_config.json")
DRIVE_MAPP_PARAMS_PATH = os.path.join(BASE_DIR, "drive_mapp_params.yaml")

def load_drive_mapp_config():
    try:
        with open(DRIVE_MAPP_CONFIG_PATH, 'r') as f:
            return json.load(f)
    except Exception:
        return {"department_equipment": {}, "equipment_overrides": {}, "vfd_connection": {"mock_mode": True}}

def load_drive_mapp_params():
    try:
        import yaml as _yaml
        with open(DRIVE_MAPP_PARAMS_PATH, 'r') as f:
            return _yaml.safe_load(f)
    except Exception:
        return {"monitor_params": [], "manager_params": [], "tse_params": []}

def save_drive_mapp_params(params_data):
    import yaml as _yaml
    with open(DRIVE_MAPP_PARAMS_PATH, 'w') as f:
        _yaml.dump(params_data, f, allow_unicode=True, sort_keys=False)

def _slugify_dm(text):
    import re
    return re.sub(r'[^\w\-]+', '-', text).strip('-').lower()

def _count_equipment_from_template(template_filename):
    """Parse a department template to count SAFs, RAFs, and washer pumps from OPC UA node IDs."""
    import re as _re
    template_path = os.path.join(BASE_DIR, 'templates', 'iot', 'departments', template_filename)
    try:
        with open(template_path, 'r', encoding='utf-8') as f:
            content = f.read()
        saf = len(_re.findall(r'"DI_\w+_SAFan\w*_ON"', content))
        raf = len(_re.findall(r'"DI_\w+_RetAFan\w*_ON"', content))
        wp  = len(_re.findall(r'"DI_\w+_WasherPump\w*_ON"', content))
        return saf, raf, wp
    except Exception:
        return 0, 0, 0

def _generate_dept_equipment(dept, cfg):
    """Return list of equipment dicts for a department, counts derived from its template."""
    dept_name = dept['category']
    overrides = cfg.get('equipment_overrides', {}).get(dept_name, {})
    conn = cfg.get('vfd_connection', {})

    template_file = dept.get('template', '')
    n_saf, n_raf, n_wp = _count_equipment_from_template(template_file) if template_file else (0, 0, 0)

    def _make_equip(name, slug):
        ov = overrides.get(slug, {})
        return {
            'name': name,
            'slug': slug,
            'vfd_number': ov.get('vfd_number', ''),
            'serial_number': ov.get('serial_number', ''),
            'ip_address': ov.get('ip_address', conn.get('default_ip', '192.168.1.20')),
            'tcp_port': ov.get('tcp_port', conn.get('default_port', 502)),
            'slave_address': ov.get('slave_address', conn.get('default_slave_address', 1)),
            'com_port': ov.get('com_port', conn.get('default_com_port', 'COM1')),
            'baudrate': ov.get('baudrate', conn.get('default_baudrate', 9600)),
        }

    equipment = []
    for i in range(1, n_saf + 1):
        label = f"Supply Air Fan {i}" if n_saf > 1 else "Supply Air Fan"
        equipment.append(_make_equip(label, _slugify_dm(label)))
    for i in range(1, n_raf + 1):
        label = f"Return Air Fan {i}" if n_raf > 1 else "Return Air Fan"
        equipment.append(_make_equip(label, _slugify_dm(label)))
    for i in range(1, n_wp + 1):
        label = f"Washer Pump {i}" if n_wp > 1 else "Washer Pump"
        equipment.append(_make_equip(label, _slugify_dm(label)))
    return equipment

def _get_tse_departments():
    """Return list of dept dicts from TSE submodule entries in input.yaml."""
    data = load_data()
    seen = set()
    depts = []
    for sub in data.get('submodules', []):
        ...
        if cat and cat not in seen:
            ...
            depts.append({
                'category': cat,
                'name': sub.get('name', cat),
                'slug': _slugify_dm(cat),
                'template': sub.get('template', ''),
                'node_prefix': sub.get('node_prefix', ''),
            })
    return depts

def _resolve_equip(dept_slug, equip_slug):
    """Return (dept_dict, equip_dict, cfg) or (None, None, cfg) on miss."""
    cfg = load_drive_mapp_config()
    dept = next((d for d in _get_tse_departments() if d['slug'] == dept_slug), None)
    if not dept:
        return None, None, cfg
    equipment = _generate_dept_equipment(dept, cfg)
    equip = next((e for e in equipment if e['slug'] == equip_slug), None)
    return dept, equip, cfg

class VFDInterface:
    """Modbus interface for a single VFD. Tries LAN (TCP) first, then Serial."""

    def __init__(self, ip_address, tcp_port, slave_address, com_port, baudrate, mock_mode=False):
        self.ip_address = ip_address
        self.tcp_port = tcp_port
        
        if isinstance(slave_address, str):
            try:
                self.slave_address = int(slave_address, 16)
            except ValueError:
    ...
    def _connect(self):
        print(f"🔄 VFDInterface connecting to IP: {self.ip_address}:{self.tcp_port}, Slave: {self.slave_address}, COM: {self.com_port}")
        try:
            from pyModbusTCP.client import ModbusClient
            self.tcp_client = ModbusClient(
                host=self.ip_address, port=self.tcp_port,
                unit_id=self.slave_address, auto_open=True, timeout=2.0
            )
            if self.tcp_client.open():
                self.connection_type = 'lan'
                self.connected = True
                print(f"✅ VFD connected via LAN!")
                return
            else:
                print(f"⚠️ VFD LAN connection failed (open returned False).")
        except Exception as e:
            print(f"⚠️ VFD LAN exception: {e}")
            pass
        try:
            import minimalmodbus, serial as _serial
            inst = minimalmodbus.Instrument(self.com_port, self.slave_address)
            inst.serial.baudrate = self.baudrate
            inst.serial.parity = _serial.PARITY_NONE
            ...
            self.connected = True
            print(f"✅ VFD connected via Serial!")
        except Exception as e:
            print(f"❌ VFD Serial connection failed: {e}")
            self.connected = False

    def read_register(self, hex_address, scale=1):
        if self.mock_mode:
            return 0
        ...
        if not self.connected:
            return None
        try:
            addr_int = int(hex_address, 16)
            if self.connection_type == 'lan':
                regs = self.tcp_client.read_holding_registers(addr_int, 1)
                if not regs:
                    raise Exception("no data (read_holding_registers returned None/empty)")
                raw = regs[0]
            else:
                raw = self.serial_inst.read_register(addr_int, 0)
            val = raw * scale
            return round(val, 2) if isinstance(val, float) else val
        except Exception as e:
            print(f"⚠️ VFD Read Error at {hex_address} via {self.connection_type}: {e}")
            self.connected = False
            return None

    def write_register(self, hex_address, value, scale=1):
        if self.mock_mode:
            return True
        if not self.connected:
            return False
        try:
            raw = int(float(value) / scale)
            addr_int = int(hex_address, 16)
            if self.connection_type == 'lan':
                if not self.tcp_client.write_single_register(addr_int, raw):
                    raise Exception("write_single_register returned False")
            else:
                self.serial_inst.write_register(addr_int, raw, 0)
            print(f"✅ VFD Write Success at {hex_address} (Raw: {raw}) via {self.connection_type}")
            return True
        except Exception as e:
            print(f"❌ VFD Write Error at {hex_address} via {self.connection_type}: {e}")
            self.connected = False
            return False

    def close(self):
        if self.connection_type == 'lan' and self.tcp_client:
            try:
                self.tcp_client.close()
            except Exception:
                pass
        elif self.connection_type == 'serial' and self.serial_inst:
            try:
...
def _make_vfd_interface(equip, cfg):
    conn = cfg.get('vfd_connection', {})
    return VFDInterface(
        ip_address=equip.get('ip_address', conn.get('default_ip', '192.168.1.20')),
        tcp_port=equip.get('tcp_port', conn.get('default_port', 502)),
        slave_address=equip.get('slave_address', conn.get('default_slave_address', 1)),
        com_port=equip.get('com_port', conn.get('default_com_port', 'COM1')),
        baudrate=equip.get('baudrate', conn.get('default_baudrate', 9600)),
        mock_mode=conn.get('mock_mode', False)
    )

# --- Drive Mapp page routes ---

@app.route('/drivemapp')
def drivemapp_index():
    depts = _get_tse_departments()
    cfg = load_drive_mapp_config()
    dept_rows = []
    for dept in depts:
        for equip in _generate_dept_equipment(dept, cfg):
            dept_rows.append({'dept': dept, 'equip': equip})
    return render_template('drivemapp/index.html', dept_rows=dept_rows, tse_depts=depts)

@app.route('/drivemapp/<dept_slug>')
def drivemapp_department(dept_slug):
    dept = next((d for d in _get_tse_departments() if d['slug'] == dept_slug), None)
    if not dept:
        return "Department not found", 404
    cfg = load_drive_mapp_config()
    equipment = _generate_dept_equipment(dept, cfg)
    return render_template('drivemapp/department.html', dept=dept, equipment=equipment)

@app.route('/drivemapp/<dept_slug>/<equip_slug>')
def drivemapp_equipment(dept_slug, equip_slug):
    dept, equip, _ = _resolve_equip(dept_slug, equip_slug)
    if not dept:
        return "Department not found", 404
    if not equip:
        return "Equipment not found", 404
    params = load_drive_mapp_params()
    role = session.get('role')
    if role == 'Tse':
        config_params = params.get('tse_params', [])
    elif role == 'Manager':
        config_params = params.get('manager_params', [])
    else:
        config_params = []
    return render_template(
        'drivemapp/equipment.html',
        dept=dept, equip=equip,
        monitor_params=params.get('monitor_params', []),
        config_params=config_params,
        user_role=role
    )

@app.route('/drivemapp/settings')
def drivemapp_settings():
    if session.get('role') != 'Tse':
        return redirect('/drivemapp')
    params = load_drive_mapp_params()
    depts = _get_tse_departments()
    return render_template(
        'drivemapp/settings.html',
        params=params,
        tse_depts=depts
    )

# --- Drive Mapp API routes ---

@app.route('/api/drivemapp/get_value/<dept_slug>/<equip_slug>/<hex_addr>')
def drivemapp_get_value(dept_slug, equip_slug, hex_addr):
    _, equip, cfg = _resolve_equip(dept_slug, equip_slug)
    if not equip:
        return jsonify({'error': 'Not found'}), 404
    params = load_drive_mapp_params()
    all_params = params.get('monitor_params', []) + params.get('manager_params', []) + params.get('tse_params', [])
    scale = next((p['scale'] for p in all_params if p['address_hex'] == hex_addr), 1)
    vfd = _make_vfd_interface(equip, cfg)
    val = vfd.read_register(hex_addr, scale)
    vfd.close()
    return jsonify({'address': hex_addr, 'value': val})

@app.route('/api/drivemapp/set_value', methods=['POST'])
def drivemapp_set_value():
    if 'userloggedin' not in session:
        return jsonify({'success': False, 'error': 'Unauthorized'}), 401
    data = request.json or {}
    _, equip, cfg = _resolve_equip(data.get('dept_slug', ''), data.get('equip_slug', ''))
    if not equip:
        return jsonify({'success': False, 'error': 'Not found'}), 404
    hex_addr = data.get('address', '')
    value = data.get('value')
    params = load_drive_mapp_params()
    all_params = params.get('manager_params', []) + params.get('tse_params', [])
    scale = next((p['scale'] for p in all_params if p['address_hex'] == hex_addr), 1)
    vfd = _make_vfd_interface(equip, cfg)
    success = vfd.write_register(hex_addr, value, scale)
    vfd.close()
    return jsonify({'success': success})

@app.route('/api/drivemapp/save_info', methods=['POST'])
def drivemapp_save_info():
    """Tse only: save vfd_number / serial_number for a specific equipment."""
    if session.get('role') != 'Tse':
        return jsonify({'success': False, 'error': 'Unauthorized'}), 401
    data = request.json or {}
    dept_slug = data.get('dept_slug', '')
    equip_slug = data.get('equip_slug', '')
    dept = next((d for d in _get_tse_departments() if d['slug'] == dept_slug), None)
    if not dept:
        return jsonify({'success': False, 'error': 'Department not found'}), 404
    cfg = load_drive_mapp_config()
    overrides = cfg.setdefault('equipment_overrides', {}).setdefault(dept['category'], {}).setdefault(equip_slug, {})
    for str_field in ('vfd_number', 'serial_number', 'ip_address', 'com_port'):
        if str_field in data:
            overrides[str_field] = data[str_field]
    for int_field in ('tcp_port', 'slave_address', 'baudrate'):
        if int_field in data:
            try:
                overrides[int_field] = int(data[int_field])
            except (ValueError, TypeError):
                pass
    with open(DRIVE_MAPP_CONFIG_PATH, 'w') as f:
        json.dump(cfg, f, indent=2)
    return jsonify({'success': True})


@app.route('/api/drivemapp/save_params', methods=['POST'])
def drivemapp_save_params():
    """Tse only: update drive_mapp_params.yaml."""
    if session.get('role') != 'Tse':
        return jsonify({'success': False, 'error': 'Unauthorized'}), 401
    data = request.json or {}
    allowed_sections = ('monitor_params', 'manager_params', 'tse_params')
    params = load_drive_mapp_params()
    for section in allowed_sections:
        if section in data:
            params[section] = data[section]
    save_drive_mapp_params(params)
    return jsonify({'success': True})
