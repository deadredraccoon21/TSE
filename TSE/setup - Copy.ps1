...
function ensure_pip_installed {
    ...
    if (!(command_exists "pip")) {
        ...
        try {
            ...
            # --- CORRECTED: Added -ErrorAction Stop to ensure the catch block is triggered on failure ---
            Invoke-WebRequest -Uri $getPipUri -OutFile $getPipScript -ErrorAction Stop
            
            # --- CORRECTED: Removed incorrect check of $LASTEXITCODE for a PowerShell cmdlet ---
...
# --- FIX: Set security protocol to TLS 1.2 for all web requests in this session ---
# This is required for downloading from modern secure servers (like pypa.io for pip).
[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
Write-Output "Set security protocol to TLS 1.2 for web requests."
