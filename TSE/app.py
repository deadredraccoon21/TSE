    ...
    if 'userloggedin' in session:
        ...
        if 'last_activity' in session:
            try:
                ...
                # If inactive for more than 2 minutes (120 seconds)
...
def save_alarms_to_list_yaml(current_alarms_from_source):
    with alarm_list_lock:
        try:
            # 1. Load persistent data from the YAML file
            ...
            for alarm_in_list in persistent_alarms:
                ...
                if is_unresolved and code in current_trip_map and not current_trip_map[code]:
                    ...
                    # Use platform-compatible format codes (removed '-')
                    alarm_in_list['resolved_time'] = datetime.now().strftime("%m/%d/%Y, %I:%M:%S %p")

            # 3. Check for NEWLY TRIGGERED alarms
            for source_alarm in current_alarms_from_source:
                ...
                if current_trip and not previous_trip:
                    ...
                    # Make time parsing robust
                    if 'time' in new_alarm_entry and new_alarm_entry['time']:
                        try:
                            # This part now only runs if needed and is safer
                            dt = datetime.strptime(new_alarm_entry['time'], "%Y-%m-%d %H:%M:%S")
                            # Use platform-compatible format codes (removed '-')
                            new_alarm_entry['time'] = dt.strftime("%m/%d/%Y, %I:%M:%S %p")
                        except (ValueError, TypeError):
                            # If parsing fails, just use the original time string or set a new one
                            new_alarm_entry['time'] = datetime.now().strftime("%m/%d/%Y, %I:%M:%S %p")
            ...
            # Use a more informative error print
            import traceback
...
def user_login():
    if request.method == 'POST':
        ...
        if user_role:
            ...
            # Set the last activity timestamp on successful login
            session['last_activity'] = datetime.now().isoformat()
...
def logout():
    ...
    session.pop('role', None)
    session.pop('allowed_submodules', None)
    session.pop('last_activity', None)  # Clear the inactivity tracker
    flash("You have been successfully logged out.", "info")
    return redirect(url_for('user_login'))  # Redirect to login page after logout
