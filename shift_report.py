...
# Filters alarms exactly within a shift's start and end time, and applies severity filtering
def get_shift_alarms(base_dir, from_time, to_time):
    path = os.path.join(base_dir, 'alarmList.json')
    shift_alarms = []
    if not os.path.exists(path): return []
    
    shift_data = load_shift_data(base_dir)
    global_config = shift_data.get("global_config", {})
    
    # 1. Master kill-switch
    if not global_config.get("include_alarms", True):
        return []
        
    # 2. Get the allowed severities array
    selected_severities = [str(x) for x in global_config.get("selected_severities", [])]
    ...
    try:
        with open(path, 'r') as f:
            ...
            for alarm in data.get('alarms', []):
                ...
                # Check if severity is in our selected array
                alarm_sev = str(alarm.get('severity', ''))
                if selected_severities and alarm_sev not in selected_severities:
                    continue
                ...
                try:
                    ...
                    a_time = a_time_full[:5]
                    
                    if is_overnight:
                        if (a_date == yesterday_str and a_time >= from_time) or (a_date == today_str and a_time <= to_time):
...
def generate_daily_report_pdf(client_name, date_str, multi_shift_data, selected_fields):
    ...
    shift_data = load_shift_data(base_dir) # Add this
    ...
    html = render_template(
        ...
        global_config=shift_data.get("global_config", {}) # Pass the config to the PDF!
    )
    # --- PDF GENERATION ---
    try:
...
def send_email_report(to_email, pdf_bytes, report_date, report_title="Daily_Report", is_delayed=False, scheduled_time=''):
    ...
    # Get client_name from the same config object
   ...
   # 2. HTML Signature - Streamlined for exact alignment
    ...
    <html>
      <body style="font-family: Arial, sans-serif; color: #333; line-height: 1.6;">
        <p>Hello,</p>
        <p>Please find attached the Daily plant report for <b>{report_date}</b>.</p>
        <br>
        <p>With Regards,</p>
